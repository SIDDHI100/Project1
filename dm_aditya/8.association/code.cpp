...
set<string> products;
map<string, int> freq;double confidence;
...
{
set<string> dup;for (set<string>::iterator it = arr.begin(); it != arr.end();
it++)
dup.insert(*it);
return dup;
}
...
return;
int count1 = countOccurences(items); // Total supportint count2 = countOccurences(v1);
double conf = (((double)count1) / count1) * 100;
if (conf >= confidence)
{
fw1 << "{ ";
for (auto s : v1)
